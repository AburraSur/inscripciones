...
$(document).ready(function(){
		...
		$.ajax({
			...
			success: function ( data ) {
				...
				if (id.resp == 1 ){
					...
					if(esc == 1){
						open('./pdf2/escarapela.php?ideve='+<?php echo $ideve; ?>+'&tp=1&idasis='+idper, '_blank');
					}
	...
	$('.reemp').click(function(){
		...
		$("#divasis").html( '<fieldset><legend><i>Reemplazar Asistencia</i></legend><fieldset><legend><i>Inscrito</i></legend><table width="60%" align="center"	><tr><td><font face="Verdana" size="2"  ><b><i>Identificacion:</i></b></font></td>		<td><font face="Verdana" size="2"  ><b>'+ced+'</b><input type=hidden name=oldid value="'+ced+'" /></font></td></tr><tr><td><font face="Verdana" size="2"  ><b><i>Nombres:</i></b></font></td><td>	<font face="Verdana" size="2"  ><b>'+nom+'</b></font> </td>	<td><font face="Verdana" size="2"  ><b><i>Apellidos:</i></b></font></td><td><font face="Verdana" size="2"  ><b>'+ape+'</b></font></td></tr></table></fieldset><fieldset><legend><i>Reemplazo</i></legend><table width=90% align=center id=cuerpoins	><tbody><tr><td><table width=90% align=center id=cuerpoins	><tbody><tr><td><font face=Verdana size=2  ><b><i>Documento<br>de Identidad:</i></b></font></td><td><br><input type=text name=docid1 id=docid1 size=32 class="required number" /><font color=red >*</font></td></tr><tr><td><font face=Verdana size=2  ><b><i>Nombres<br>completos:</i></b></font></td><td><br><input type=text name=nombres1 id=nombres1 size=32 class="required"  /><font color=red >*</font><input type=hidden name=oldced /><input type="hidden" class="docid1" value="1" /><input type=hidden name=ideve id=ideve value="'+<?php echo $ideve; ?>+'"  /><input type=hidden name=idmod id=idmod value="'+<?php echo $idmod; ?>+'"  /></td><td><font face=Verdana size=2  ><b><i>Apellidos<br>completos:</i></b></font></td><td><br><input type=text name=apellidos1 id=apellidos1 size=32  /><font color=red >*</font></td></tr><tr><td><font face=Verdana size=2  ><b><i>E-mail:</i></b></font></td><td><input type=text name=mail1 id=mail1 size=32 class="required mail" />	</td><td><font face=Verdana size=2  ><b><i>Tel&eacute;fono:</i></b></font></td><td><input type=text name=tel1 size=25 class=required /><font color=red >*</font>&nbsp;<font face=Verdana size=2  ><b><i>Ext.</i></b></font><input type=text name=ext1 id=ext1 size=4 /></td></tr><tr><td><font face=Verdana size=2><b><i>Celular:</i></b></font></td><td><input type=text name=cel1 id=cel1 size=32 /></td><td><font face=Verdana size=2  ><b><i>Municipio:</i></b></font></td><td><select name=muni1 id=muni1 ><option>--Seleccione--</option><option>CALDAS</option><option>ENVIGADO</option><option>ITAGUI</option><option>LA ESTRELLA</option><option>SABANETA</option><option>MEDELLIN</option></select><input type=hidden name=typ value="0" /></td></tr><tr><td><font face=Verdana size=2  ><b><i>Cargo:</i></b></font></td><td><input type=text name=cargo1 id=cargo1 size=32 /></td><td><font face=Verdana size=2  ><b><i>Habeas Data:</i></b></font></td><td><font face=Verdana size=2  ><b><i>Si</i></b></font><input type=radio name=habeas class=habeas id=SI1 /><font face=Verdana size=2  ><b><i>NO</i></b></font><input type=radio name=habeas class=habeas id=NO1 /><font color=red >*</font></td></tr></table><br><center><input type=image src=./img/acpt.png id=acpt /></center></fieldset>');
	...
	$(".cancel").click(function(){
        ...
        if( ans == 1 ){
			$.blockUI({ message: '<font size=2 >Motivo de Cancelacion:</font><br><textarea name=motiv id=motiv cols=30 rows=6 ></textarea> <br><button id=mot >Aceptar</button>' });
				$('#mot').click(function(){
					var mot = $('#motiv').val();
					...
					$.ajax({
						...
						type: 'POST',
						data: {idcancel: idcancel , ideve: ideve , idmod: idmod , i: ic , typ: 2 , mot: mot},
						success: function ( data ) {
							var id = eval ( '(' + data + ')' );
...
if($pago == 1){
	$sqlpay = $db->consulta("select a.cedula,a.nombres,a.apellidos,ea.e_asistio 'asis',ea.id 'cod',a.comentario 'coment',p.tarifa 'tarifa',p.vlrpago 'vlrp' from asistentes a inner join event_asist ea inner join pagos p where ea.e_asistio=0 and a.cedula=ea.cedula and p.vlrpago>0 and p.cedula=ea.cedula and p.idevento=ea.idevent and ea.idevent=$ideve order by a.apellidos ASC");
	
	$sqlinv = $db->consulta("select a.cedula,a.nombres,a.apellidos,ea.e_asistio 'asis',ea.id 'cod',a.comentario 'coment',p.tarifa 'tarifa',p.vlrpago 'vlrp' from asistentes a inner join event_asist ea inner join pagos p where ea.e_asistio=0 and a.cedula=ea.cedula and p.tarifa='Invitado' and p.cedula=ea.cedula and p.idevento=ea.idevent and ea.idevent=$ideve order by a.apellidos ASC");
	...
	<thead bgcolor=gray >
		...
		<th><font face=Verdana size=4 ><b><i>Valor Pago</b></font></th>
...
";
	...
	while($rowinv = $db->fetch_array($sqlinv)){
		echo "<tr align=center >
			...
			<td><font face=Verdana size=2 ><span id=tar$i value='$rowinv[tarifa]'  >$rowinv[tarifa]</span></font></td>
			<td><font face=Verdana size=2 ><span id=vlrp$i value='$rowinv[vlrp]'  >$rowinv[vlrp]</span></font></td>
	...
	while($rowpay = $db->fetch_array($sqlpay)){
		echo "<tr align=center >
			...
			<td><font face=Verdana size=2 ><span id=tar$i value='$rowpay[tarifa]'  >$rowpay[tarifa]</span></font></td>
			<td><font face=Verdana size=2 ><span id=vlrp$i value='$rowpay[vlrp]'  >$rowpay[vlrp]</span></font></td>
...
if ( $typ == 1 ){
	...
	if( $pay == 1 ){
		$camp = ", p.tarifa,p.vlrpago";
		$inn = "inner join pagos p";
		$add = "and p.cedula=ea.cedula and p.idevento=ea.idevent";
	}else{
		$inn = "";
		$add = "";
	}
	if( $n == 0){
		$sql = $db->consulta("select a.cedula,a.nombres,a.apellidos,a.email,ea.e_asistio 'asis',ea.id 'cod', ea.observa_admin 'motivo',a.comentario 'coment' from asistentes a inner join event_asist ea where a.cedula=ea.cedula and ea.idevent=$ideve order by a.apellidos ASC ");
	}elseif( $n == 1){
		...
		$sql = $db->consulta("select a.cedula,a.nombres,a.apellidos,a.email,ea.e_asistio 'asis',ea.id 'cod', ea.observa_admin 'motivo',a.comentario 'coment' $camp from asistentes a inner join event_asist ea $inn where a.cedula=ea.cedula $add and ea.idevent=$ideve and ea.cedula='$id' ");
		//$echosql = "select a.cedula,a.nombres,a.apellidos,a.email,ea.e_asistio 'asis',ea.id 'cod',a.comentario 'coment' from asistentes a inner join event_asist ea $inn where a.cedula=ea.cedula $add and ea.idevent=$ideve and ea.cedula='$id'";
	}elseif( $n == 2){
		$sql = $db->consulta("select a.cedula,a.nombres,a.apellidos,a.email,ea.e_asistio 'asis',ea.id 'cod', ea.observa_admin 'motivo',a.comentario 'coment' $camp from asistentes a inner join event_asist ea $inn where a.cedula=ea.cedula $add and ea.idevent=$ideve and a.nombres like'%$id%' ");
	}elseif( $n == 6){
		$sql = $db->consulta("select a.cedula,a.nombres,a.apellidos,a.email,ea.e_asistio 'asis',ea.id 'cod', ea.observa_admin 'motivo',a.comentario 'coment' $camp from asistentes a inner join event_asist ea $inn where a.cedula=ea.cedula $add and ea.idevent=$ideve and a.apellidos like'%$id%' order by apellidos ASC ");
	}else{
		$sql = $db->consulta("select a.cedula,a.nombres,a.apellidos,a.email,ea.e_asistio 'asis',ea.id 'cod', ea.observa_admin 'motivo',a.comentario 'coment' $camp from asistentes a inner join event_asist ea $inn where a.cedula=ea.cedula $add and ea.idevent=$ideve and ea.id='$id'");
	}
	
}else{
	
	if( $n == 0){
		$sql = $db->consulta("select a.cedula,a.nombres,a.apellidos,a.email,ma.asistio 'asis',a.comentario 'coment' from asistentes a inner join mod_asis ma where a.cedula=ma.cedula and ma.idmod=$idmod  and ma.idevento=$ideve order by a.apellidos ASC");
	}elseif( $n == 1){
...
if( $num == 0 ){
	echo "<center><img src=./img/folder.png /><br><font face=Verdana size=6 ><b><i>El Usuario $id no se Encuentra registrado para este evento</i></b></font></center> ";
}else{
echo "<div id=listado >
	...
	<thead bgcolor=gray >
		...
		<th><font face=Verdana size=4 ><b><i>Nombres</b></font></th>
		<th><font face=Verdana size=4 ><b><i>Apellidos</b></font></th>";
		if( $pay == 1 ){echo "<th><font face=Verdana size=4 ><b><i>Tarifa</b></font></th><th><font face=Verdana size=4 ><b><i>Valor Pago</b></font></th>";}
echo "  <th><font face=Verdana size=4 ><b><i>Comentario</b></font></th>
		<th><font face=Verdana size=4 ><b><i>Asiste</b></font></th>		
		</thead>
	<tbody>
";
...
while( $row = $db->fetch_array($sql) ){
	...
	if( $row['asis'] != 0 ){
		...
		if( $row['asis'] == 1){
			...
			echo "<center><img src=./img/folder.png /><br><font face=Verdana size=6 ><b><i>El Usuario con Identificacion $id Cancelo la  Asistencia al Evento</font><br><font size=2 >NOTA: $row[motivo]</i></b></font></center>";
		}
          }
	}else{	
	echo "<tr align=center >
			...
			if( $pay == 1 ){echo "<td><font face=Verdana size=2 ><span id=tar$i value='$row[tarifa]'  >$row[tarifa]</span></font></td><td><font face=Verdana size=2 ><span id=pag$i value='$row[vlrpago]'  >$row[vlrpago]</span></font></td>";}
echo " 
