$(document).ready(function() {
  ...
  $("#boton").mouseover(function(){
	...
	$('#all').click(function(){
		var year = $('#year2').val();
		window.open('./informes/totalasis.php?year='+year, '_blank');
	});
...
$(document).ready(function(){
    ...
    $("#form1").submit(function(){
		...
		$.ajax({
			...
			$.blockUI({ message: '<h3><img src=./img/check2.png /><br>Registro Exitoso.<br>a Continuacion podras visualizar el link del formulario de registro</h3>' });
				...
				$("#mail").append("<input type=text size=100 name=link id=link value='http://http://apps.ccas.org.co/inscripciones/maestro/confor3.php?cod="+obj.capa+"'/>");
				 $('#link').focus();
		...
		if( ideve != 0 ){
			...
			success: function ( data ) {
				...
				if( obj.resp == 'SI'){
					if( idmod != 0 ){
						if( id != 'Identificacion' ){
							open('./pdf2/escarapela.php?ideve='+ideve+'&idmod='+idmod+'&tp=2&idasis='+id, '_blank');
						}else{
					...
					open('./pdf2/escarapela.php?ideve='+ideve+'&tp=1&idasis='+id, '_blank');
				}
						...
						if(n==3){
							$('#formedit'+objet).show('fast');
								...
								if(objet==2){
									...
									$('#estevent').append('<option selected >'+objx.dt15+'</option>');
									if(objx.dt7 != ''){
								...
								if(objet==3){
									$('#modu1').attr('value',objx.dt1);
									$('#fmod1').attr('value',objx.dt2);
									$('#idmod').attr('value',objx.dt3);	
								}
							}
							if(n==4){
								$('#divedit'+objet).html('');
									...
									$.ajax({
										...
										success: function( dataf ){
											...
											if(objf.sw==1){
													...
													if(objet==3){
														$('#modu1').attr('value',objf.dt1);
														$('#fmod1').attr('value',objf.dt2);	
														$('#idmod').attr('value',objf.dt3);									
													}
											}
										}
									})
	...
	$(".searchresult").submit(function(){
		$("#s3").attr('value','Identificacion').addClass('lett');
		$("#s4").attr('value','Nombre').addClass('lett');
		$.ajax({
			url: './insert/updateasis.php',
			type: 'POST',
			data: $(this).serialize(),
			success: function( data ){
				...
				if( obj.sw == true ){
					...
					setTimeout($.unblockUI, 3000); 
					$('form.searchresult').each (function(){
						this.reset();					
					});
					$('.searchresult').hide('fast');
					$('#objet option[value=5]').attr('selected',true);
				}else{
					//alert("Ocurrio un Problema Durante la Cancelacion de la Inscripcion, Por Favor Comuniquese con el Administrador del Sistema");
	...
	$('#eliminado').click(function(){
		...
		$.blockUI({ message: '<img src=./img/login.png /><img src=./img/close.png id=out align=right /><br><font size=2 >Usuario:</font>&nbsp;<input type=text name=user id="usuario" /><br><font size=2 >Clave:</font>&nbsp;&nbsp;&nbsp;<input type=password name=pass id=pass /><br><button id=okp >Aceptar</button>' });
		$('#usuario').bind('blur', function() {
			var up = $(this).val().toUpperCase();
			$(this).attr('value',up);
		});
	...
	$('.subreg').click(function(){
		var idf = $(this).attr('value');
		$.ajax({
			url: './insert/newreg.php',
			type: 'POST',
			data: $('#'+idf).serialize(),
			success: function ( datar ){
				var objr = eval ( '(' + datar + ')' );
				if(objr.sw==1){
					$.blockUI({ message: '<h3><img src=./img/check2.png /><br>'+objr.msn+'</h3>' });
					setTimeout($.unblockUI, 3000); 
					$('#'+idf).each (function(){
						this.reset();					
					})
					...
					$.blockUI({ message: '<h3><img src=./img/bad.png /><br>'+objr.msn+'</h3>' });
					setTimeout($.unblockUI, 3000); 
				}				
			}
		})
	...
	$('#findpart').click(function(){
		var idevent = $('#eventch').val();
		var idpart = $('#cedpart').val();
		$.ajax({
			url: './js/find2.php',
			type: 'GET',
			data: {n:5,idevent:idevent,idpart:idpart},
			success: function ( datac ){
				var objc = eval ( '(' + datac + ')' );
				if(objc.sw==1){
					$('#spart').html(objc.part);
					$('#sempr').html(objc.rsoc);
					$('#actnit').attr('value',objc.nit);
					$('#relacion').show('fast');
				}else{
