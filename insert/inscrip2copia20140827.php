			...
			$who = $_POST["who$k"];
				...
				if($numask==0){
					...
					if( $rowsql['modulos'] == 'SI' ){
						$coddm = "<tr><td colspan=2 >Módulos a los Cuales Asistirá</td></tr><tr><td>Fecha</td><td>Módulo</td></tr> ";
						for($j=1;$j<=$_POST['ctrlmod'];$j++){
							if( $_POST["mod$j"] != null ){
								...
								$sqlmod = $db->consulta2("select modulo,fec_mod from modulos where idmod='$_POST[mod$j]' ");
								$coddm.= "<tr><td>$sqlmod[0]</td><td>$sqlmod[1]</td></tr> ";
							}
						}
					}
				}
...
//Si el evento al que se ha inscrito tiene costo, una de las Asesoras Empresariales de la Cámara de Comercio Aburrá Sur le contactará para verificar los datos relacionados con el pago de su inscripción.
				...
				require('./class.phpmailer.php');
					...
					if( $eventasis != 1 ) {
						...
						$body2 = "<html><head><style>p{font-family:Arial;font-size:12px}</style></head><body>Hay un Nuevo Registro en el Evento $rowmail[nom_evento]<br><br>Empresa: $nomemp &nbsp;&nbsp; NIT: $nit<br><br>Inscritos:<br>  $todos </body>";
