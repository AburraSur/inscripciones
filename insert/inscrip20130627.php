	...
	if($numrows == 0){
		...
		if($ctrl2 == $ctrl){
			...
			$cargo=$_POST["cargo$k"];
				...
				if($yaregistra == 0){
				$asist = $db->consulta("insert into asistentes (cedula,nombres,apellidos,email,tel,ext,cel,municipio,nit,cargo) values ('$ced','$nombres','$apellidos','$mailx','$tel','$ext','$cel','$muni','$nit','$cargo')");
				$db->consulta("insert into log (fechalog,descripcion) values ('$fecha','El Usuario $varsesion creo el asistente $ced en el evento $idev ')");
					...
					$asist = $db->consulta("update asistentes set nombres='$nombres',apellidos='$apellidos',email='$mailx',tel='$tel',ext='$ext',cel='$cel',municipio='$muni',nit='$nit',cargo='$cargo' where cedula='$ced'");
...
<br>
Tenga en cuenta que en nuestro Centro de Convenciones tenemos a su disposición el servicio de Parqueadero Cubierto administrado por ZONA CENTRO.
<br>
