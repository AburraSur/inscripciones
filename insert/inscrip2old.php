...
if($numins < $rowsql['cupo'] ){
	...
	if($numrows == 0){
		...
		if($ctrl2 == $ctrl){
			...
			if($yaregempre == 0){
				$empresa = $db->consulta("insert into empresa (nit,rsocial,dir,comentario) values ('$nit','$_POST[nomemp]','$_POST[diremp]','$coment')");				
			...
			$cargo=$_POST["cargo$k"];
				...
				if($yaregistra == 0){
				$asist = $db->consulta("insert into asistentes (cedula,nombres,apellidos,email,tel,ext,cel,municipio,cargo,comentario) values ('$ced','$nombres','$apellidos','$mailx','$tel','$ext','$cel','$muni','$cargo','$coment')");
