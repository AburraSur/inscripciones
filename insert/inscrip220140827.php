	...
	if($numrows == 0){
		...
		if($sw == 2){
			...
			$who = $_POST["who$k"];
				...
				*/
				...
				if($numask==0){
					...
					if( $rowsql['modulos'] == 'SI' ){
						$coddm = "<tr><th>Módulo</th><th>Fecha</th></tr> ";
						for($j=1;$j<=$_POST['ctrlmod'];$j++){
							if( $_POST["mod$j"] != null ){
								$db->consulta("insert into mod_asis (idmod,cedula,idevento) values (".$_POST["mod$j"].",'$ced',$idev)");
								$db->consulta("insert into log (fechalog,descripcion) values ('$fecha','El Usuario $varsesion inscribio a $ced en modulo ".$_POST["mod$j"]." ')");
								
									$sqlmod = $db->consulta("select modulo,fec_mod from modulos where idmod='".$_POST["mod$j"]."' ");
									$rowmod=$db->fetch_array($sqlmod);									
									$coddm.= "<tr><td>".utf8_decode($rowmod[0])."</td><td align=center >".utf8_decode($rowmod[1])."</td></tr> ";
			...
			if( $mailing2 != ''){
				...
				$correo="<p style='font-family:Arial;font-size:12px;line-height:16px;align:justify'>Señor Usuario, gracias por registrase en el evento $rowmail[nom_evento], programado para el día $pru $fdia de $mes a las $rowmail[hora] en el $rowmail[lugar].
				Recuerde presentar el Código de Inscripción y estar en el sitio del evento 15 minutos antes de su inicio. 
<br><br>
</p>
...
<p>";
if( $rowsql['pago'] == 1 ){
$pdias=array("domingo","lunes","martes","miércoles" ,"jueves","viernes","sábado");
...
$correo.="Para garantizar su cupo en el evento recuerde realizar su pago y enviar el soporte a través del correo electrónico eventos@ccas.org.co
</p>";
}

$correo.="<p><b><i>Por favor no contestar a este mensaje, ha sido enviado desde una cuenta automática y no recibirá respuesta.</i></b></p>";
//Si el evento al que se ha inscrito tiene costo, una de las Asesoras Empresariales de la Cámara de Comercio Aburrá Sur le contactará para verificar los datos relacionados con el pago de su inscripción.
				...
				require('./class.phpmailer.php');
					...
					if( $eventasis != 1 ) {
						...
						$body2 = "<html><head><style>p{font-family:Arial;font-size:12px}</style></head><body>Hay un Nuevo Registro en el Evento $rowmail[nom_evento]<br><br>Empresa: $nomemp &nbsp;&nbsp; NIT: $nit<br><br>Inscritos:<br>
						<table align=center >
							<thead>
								<tr><th>Codigo</th><th>Inscrito</th></tr>
							</thead>
					...
					if( $rowmail['respon'] == 1){
						...
						//$mail2->AddAddress("auxiliar.sistemas@ccas.org.co", "Auditorios CCAS");
						 ...
						 $correos = explode(";" , $rowmail['mailresp'] );
						 foreach($correos as $mails ){
							$mail2->AddAddress("$mails", "Responsables");
						 }
